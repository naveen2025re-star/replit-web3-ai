...
export class SmartDiagnosticsParser {
    ...
    private static readonly VULNERABILITY_CATEGORIES = {
    ...
    static parseAuditReport(reportText: string, contractCode: string): SmartDiagnostic[] {
        ...
        const diagnostics: SmartDiagnostic[] = [];
     ...
     */
    private static extractLineNumber(text: string): number | null {
        const patterns = [
            /(?:line|Line|LINE)\s+(\d+)/,
            /(?:at|At|AT)\s+line\s+(\d+)/,
            /(?:on|On|ON)\s+line\s+(\d+)/,
     ...
     */
    private static determineSeverity(text: string): 'error' | 'warning' | 'information' {
        const lowerText = text.toLowerCase();
        
        for (const [keyword, severity] of Object.entries(this.SEVERITY_KEYWORDS)) {
            if (lowerText.includes(keyword)) {
                return severity as 'error' | 'warning' | 'information';
            }
        }
     ...
     */
    private static extractMessage(text: string, lineNumber: number): string {
     ...
     */
    private static categorizeVulnerability(message: string): string | undefined {
     ...
     */
    private static generateRecommendation(category: string | undefined, message: string): string | undefined {
     ...
     */
    private static calculateColumns(codeLine: string, message: string): { startCol: number; endCol: number } {
        if (!codeLine) {
     ...
     */
    private static deduplicateAndSort(diagnostics: SmartDiagnostic[]): SmartDiagnostic[] {
        // Remove duplicates based on line + message
        const unique = new Map<string, SmartDiagnostic>();
