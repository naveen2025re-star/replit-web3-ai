    ...
    try {
        ...
        const mcpTools = [
          {
            ...
            description: 'Perform comprehensive security audit of smart contract code with real-time streaming analysis',
            inputSchema: {
              type: 'object',
              properties: {
                contractCode: {
      ...
      if (action === 'call_tool' && tool) {
          ...
          switch (tool) {
            ...
            case 'analyze_multiple_contracts': {
              ...
              const factors: CreditCalculationFactors = {
                ...
                complexity: Math.min(10, Math.max(1, Math.ceil(totalLength / 1000))),
            ...
            case 'get_audit_history': {
              ...
              if (sessions.length === 0) {
                historyText += 'No audit history found. Start your first audit!';
              } else {
        ...
        res.status(400).json({
          ...
          message: 'Use action: "list_tools" to get available tools or action: "call_tool" with tool name to execute'
        });
      }
